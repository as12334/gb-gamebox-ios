...
@interface RH_HelpCenterViewController ()

@end

@implementation RH_HelpCenterViewController
...
- (void)setupInfo
{
    ...
    [self.contentTableView registerCellWithClass:[RH_HelpCenterViewCell class]] ;
...
-(NSUInteger)defaultPageSize
{
    ...
    CGFloat cellHeigh = [RH_HelpCenterViewCell heightForCellWithInfo:nil tableView:nil context:nil] ;
...
{
    [self.serviceRequest startV3HelpFirstType] ;
}
...
{
    if (type == ServiceRequestTypeV3FirstHelpFirstTyp) {
        NSArray *model = data;
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (self.pageLoadManager.currentDataCount){
        return [RH_HelpCenterViewCell heightForCellWithInfo:nil tableView:tableView context:nil] ;
    }else{
...
{
    if (self.pageLoadManager.currentDataCount){
        RH_HelpCenterViewCell *helpCenterCell = [self.contentTableView dequeueReusableCellWithIdentifier:[RH_HelpCenterViewCell defaultReuseIdentifier]] ;
...
{
    RH_HelpCenterModel *model = [self.pageLoadManager dataAtIndexPath:indexPath] ;
    NSLog(@"%@",model.mId) ;
    [self.navigationController pushViewController:[RH_HelpCenterSecondViewController viewControllerWithContext:model]
                                             animated:YES] ;
}
