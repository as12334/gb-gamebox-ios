...
@interface CLIndicateView ()
@property(nonatomic,strong,readonly) UIImageView  *imageView;
@end

@implementation CLIndicateView
{
    UIView * _contentView;
...
- (void)_init_MyIndicateView
{
    ...
    _style            = CLIndicateViewStyleActivityView;
    _topMargin        = 10.f;
    _bottomMargin     = 5.f;
...
- (void)_updateUIForKeypath:(NSString *)keyPath
{
    if ([keyPath isEqualToString:@"style"]) {
        ...
        if (_imageView) {
            ...
            if (self.style != CLIndicateViewStyleImageView) {
                return;
            }
        ...
        if (self.style == CLIndicateViewStyleCustomView) {
            [self _updateIndicateView];
        }else{
            return;
        }
    }else if ([keyPath isEqualToString:@"frame"]) {
        if (self.style != CLIndicateViewStyleActivityView || _ignoreFrameChange) {
            return;
        }
...
- (void)_updateIndicateView
{
    ...
    switch (_style) {
        case CLIndicateViewStyleActivityView:
            _indicateView = self.activityIndicatorView;
            break;

        case CLIndicateViewStyleCustomView:
            _indicateView = self.customView;
            break;

        case CLIndicateViewStyleImageView:
...
- (CLActivityIndicatorView *)activityIndicatorView
{
    if (!_activityIndicatorView) {
        ...
        _activityIndicatorView = [[CLActivityIndicatorView alloc] initWithStyle:CLActivityIndicatorViewStyleIndeterminate];
...
- (void)layoutSubviews
{
    ...
    if (CGRectGetWidth(contentRect) <= 0 || CGRectGetHeight(contentRect) <= 0) {
        ...
        if (_indicateView) {
            ...
            if (_style == CLIndicateViewStyleImageView) {
        ...
        CGSize titleLabelSize = caculaterLabelTextDrawSize(self.titleLabelText, self.titleLabelFont, contentSize.width);

        CGSize detailLabelSize = caculaterLabelTextDrawSize(self.detailLabelText, self.detailLabelFont, contentSize.width);
        ...
        if (_style == CLIndicateViewStyleImageView && _indicateView && contentSize.height > CGRectGetHeight(contentRect)) {
