...
-(void)updateViewWithInfo:(NSDictionary *)info context:(id)context
{
    ...
    if ([numStr isEqualToString:@"101"]) {
        self.chooseMoneyIcon.image = [UIImage imageNamed:@"101"];
    }
    else if ([numStr isEqualToString:@"302"])
    {
        self.chooseMoneyIcon.image = [UIImage imageNamed:@"302"];
    }
    else if ([numStr isEqualToString:@"504"])
    {
        self.chooseMoneyIcon.image = [UIImage imageNamed:@"504"];
    }
    else if ([numStr isEqualToString:@"1006"])
    {
        self.chooseMoneyIcon.image = [UIImage imageNamed:@"1006"];
    }
    else if ([numStr isEqualToString:@"4998"])
    {
        self.chooseMoneyIcon.image = [UIImage imageNamed:@"4998"];
    }
}
- (void)awakeFromNib {
    [super awakeFromNib];
