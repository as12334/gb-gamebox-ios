...
- (DCPathButtonOrientation)dcPathOrientationFromOrientation:(UIDeviceOrientation)orientation {
    ...
    if (orientation == UIDeviceOrientationPortraitUpsideDown) {
        _orientation = DCPathButtonOrientationLandscape;
        return DCPathButtonOrientationLandscape;
    }
...
- (CGPoint)createEndPointWithRadius:(CGFloat)itemExpandRadius
    ...
    switch (self.bloomDirection) {
        ...
        case kDCPathButtonBloomDirectionTop:
            ...
            return CGPointMake(self.dcButtonCenter.x + cosf((angel + 1) * M_PI) * itemExpandRadius,
                               self.dcButtonCenter.y + sinf((angel + 1) * M_PI) * itemExpandRadius);
        case kDCPathButtonBloomDirectionBottomLeft:
            
            return CGPointMake(self.dcButtonCenter.x + cosf((angel + 0.25) * M_PI) * itemExpandRadius,
                               self.dcButtonCenter.y + sinf((angel + 0.25) * M_PI) * itemExpandRadius);
            
        case kDCPathButtonBloomDirectionLeft:
           
            return CGPointMake(self.dcButtonCenter.x + cosf((angel + 0.5) * M_PI) * itemExpandRadius,
                                   self.dcButtonCenter.y + sinf((angel + 0.5) * M_PI) * itemExpandRadius);
            
        case kDCPathButtonBloomDirectionTopLeft:
            
            return CGPointMake(self.dcButtonCenter.x + cosf((angel + 0.75) * M_PI) * itemExpandRadius,
                               self.dcButtonCenter.y + sinf((angel + 0.75) * M_PI) * itemExpandRadius);
            
        case kDCPathButtonBloomDirectionBottom:
            
            return CGPointMake(self.dcButtonCenter.x + cosf(angel * M_PI) * itemExpandRadius,
                               self.dcButtonCenter.y + sinf(angel * M_PI) * itemExpandRadius);
            
        case kDCPathButtonBloomDirectionBottomRight:
            
            return CGPointMake(self.dcButtonCenter.x + cosf((angel + 1.75) * M_PI) * itemExpandRadius,
                               self.dcButtonCenter.y + sinf((angel + 1.75) * M_PI) * itemExpandRadius);
            
        case kDCPathButtonBloomDirectionRight:
            
            return CGPointMake(self.dcButtonCenter.x + cosf((angel + 1.5) * M_PI) * itemExpandRadius,
                               self.dcButtonCenter.y + sinf((angel + 1.5) * M_PI) * itemExpandRadius);
            
        case kDCPathButtonBloomDirectionTopRight:
            
            return CGPointMake(self.dcButtonCenter.x + cosf((angel + 1.25) * M_PI) * itemExpandRadius,
                               self.dcButtonCenter.y + sinf((angel + 1.25) * M_PI) * itemExpandRadius);
...
- (CAAnimationGroup *)foldAnimationFromPoint:(CGPoint)endPoint
    ...
    CGPathAddLineToPoint(path, NULL, self.dcButtonCenter.x, self.dcButtonCenter.y);
...
- (void)pathCenterButtonBloom {
    ...
    //
    NSLog(@"---00--%ld",_orientation);
    if (_orientation==DCPathButtonOrientationPortrait) {
        ...
        frame.size.width = self.bloomSize.width * 2;
        frame.size.height = self.bloomSize.height * 2;
        self.bottomView.frame = frame;
    }else{
        self.frame = CGRectMake(0, 0, self.bloomSize.height, self.bloomSize.width);
        self.center = CGPointMake(self.bloomSize.height / 2, self.bloomSize.width / 2);
        CGRect frame = self.bottomView.frame;
        frame.size.width = self.bloomSize.height * 2;
        frame.size.height = self.bloomSize.width * 2;
        self.bottomView.frame = frame;
    }
    ...
    self.pathCenterButton.center = self.dcButtonCenter;
...
- (CAAnimationGroup *)bloomAnimationWithEndPoint:(CGPoint)endPoint
    ...
    //
    CGMutablePathRef path = CGPathCreateMutable();
    CGPathMoveToPoint(path, NULL, self.dcButtonCenter.x, self.dcButtonCenter.y);
    CGPathAddLineToPoint(path, NULL, farPoint.x, farPoint.y);
