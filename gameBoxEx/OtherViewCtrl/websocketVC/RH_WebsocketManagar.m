...
@interface RH_WebsocketManagar()<SRWebSocketDelegate>
{
    int _index;
    NSTimer * heartBeat;
...
@implementation RH_WebsocketManagar

+(RH_WebsocketManagar *)instance{
    static RH_WebsocketManagar *Instance = nil;
    static dispatch_once_t predicate;
    dispatch_once(&predicate, ^{
        Instance = [[RH_WebsocketManagar alloc] init];
    });
    return Instance;
}
...
-(void)SRWebSocketOpenWithURLString:(NSString *)urlString {
    ...
    self.socket = [[SRWebSocket alloc] initWithURLRequest:
                   request];
    ;
    NSLog(@"-----%@",[RH_UserInfoManager shareUserManager].sidString);
...
-(void)sentheart{
    ...
    NSDictionary *dict = @{@"_S_COMET":@"R",@"_S_TYPE":@"PCENTER-popUp-Notice,SYS_ANN,SITE_ANN,PCENTER-dialog-Notice,MSITE-Player-Announcement-Notice,MCENTER_READ_COUNT,MSITE-ONLINERECHARGE,MSITE_DIGICCY_REFRESH_BALANCE,ECHO",@"_C_COMET":@"",@"_LOCALE_TYPE":@"zh_CN"};
    NSError *error;
