...
@interface CLBasicFileCachePool : NSObject
//根文件夹名称，由子类实现
...
/**
 * 通过文件缓存的文件夹名初始化
 * @param pathType pathType为路径类型,默认为CLPathTypeCaches
 *  cacheFileFloderName cacheFileFloderName文件缓存的文件夹名，该参数为nil则使用默认的defaultCacheFileFloderName
 */
...
//路径的类型，默认为CLPathTypeCaches
@property(nonatomic,readonly) CLPathType pathType;
 ...
 *  key key为需要缓存图片的key,不能为nil，否则将抛出异常
 *  async async指示是否异步缓存，当图片很大时，缓存图片到文件需要一段时间，会造成当前线程卡死，使用异步可解决该问题，默认为同步
 *  blockQueue blockQueue为block回调的队列对于同步操作改值无效，如果是异步且该值为nil则会使用当前队列回调
 *  completedBlock completedBlock为回调block，包含图片路径
 */
...
+ (long long)cacheFilesSizeWithPathType:(CLPathType)pathType
                    cacheFileFloderName:(NSString *)cacheFileFloderName;
