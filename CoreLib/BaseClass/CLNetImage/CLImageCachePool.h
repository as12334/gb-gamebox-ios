...
//缓存图片策略
typedef NS_OPTIONS(NSUInteger, CLCacheImagePolicy) {
    CLCacheImagePolicyNone = 0,
    CLCacheImagePolicyUseFileCache  = 1 << 0, //使用文件缓存或缓存到文件
    CLCacheImagePolicyUseOuterCache = 1 << 1, //使用外部缓存或记录外部缓存
    CLCacheImagePolicyDefault = CLCacheImagePolicyUseFileCache |
    CLCacheImagePolicyUseOuterCache //默认策略
};

//使用外部缓存
#define CacheImageUseOuterCachePolicy(_policy) (_policy & CLCacheImagePolicyUseOuterCache)
//使用文件缓存
#define CacheImageUseFileCachePolicy(_policy) (_policy & CLCacheImagePolicyUseFileCache)
...
@class CLImageCachePool;

//----------------------------------------------------------

@protocol CLImageCachePoolDelegate
 ...
 * 缓存池将要从内存移除标识为key图片
 */
- (void)imageCachePool:(CLImageCachePool *)pool willRemoveImage:(UIImage *)image andKey:(NSString *)key;
 ...
 * 缓存和获取缓存是线程安全的
 */
@interface CLImageCachePool : CLBasicFileCachePool <NSCacheDelegate>
 ...
 *  policy policy为缓存策略，默认为CLCacheImagePolicyDefault，具体见CLCacheImagePolicy定义
 *  async async指示是否异步缓存，当图片很大时，缓存图片到文件需要一段时间，会造成当前线程卡死，使用异步可解决该问题，默认为异步
 */
 ...
 *  type type为缓存图片的类型
 *  policy policy为获取缓存策略，默认为CLCacheImagePolicyDefault，具体见CLCacheImagePolicy定义
...
/**
 * 清空所有图片，内存和文件
 *  completeBlock completeBlock为删除完毕后调用的block
 */
- (void)clearAllCacheImages:(void(^)())callBackBlock;


/**
 * 代理
 */
@property(nonatomic,weak) id<CLImageCachePoolDelegate> delegate;
