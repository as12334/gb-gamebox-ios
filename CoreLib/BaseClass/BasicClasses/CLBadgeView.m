...
- (void)layoutSubviews
{
    ...
    if (self.showBadge && !self.ignoreLayout) {
...
- (void)_tapGestureHandle
{
    //判断是否响应点击
    BOOL bRet = YES;
    id<CLBadgeViewDelegate> delegate = self.delegate;
...
{
    if ([self isKindOfClass:[CLBadgeView class]]) {
        return (CLBadgeView *)self;
    }

    for (UIView * view in self.subviews.reverseObjectEnumerator) {
        CLBadgeView * badgeView = [view badgeView];
        if (badgeView != nil) {
            return badgeView;
        }
    }
...
{
    if ([self isKindOfClass:[CLBadgeView class]]) {
        [(CLBadgeView *)self vibrateWithDuration:duration];
    }
