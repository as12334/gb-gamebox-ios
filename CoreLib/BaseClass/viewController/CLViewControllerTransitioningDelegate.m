...
#import "CLPresentAnimatedTransitioning.h"
#import "MacroDef.h"
...
{
    CLBasicViewControllerAnimatedTransitioning * transitioning = [presented viewControllerAnimatedTransitioningForPresented];

    //默认动画
    transitioning = transitioning ?: [[CLPresentAnimatedTransitioning alloc] initWithType:PresentAnimatedTransitioningTypePresent];
...
{
    CLBasicViewControllerAnimatedTransitioning * transitioning = [dismissed viewControllerAnimatedTransitioningForDismissed];

    //默认动画
    transitioning = transitioning ?: [[CLPresentAnimatedTransitioning alloc] initWithType:PresentAnimatedTransitioningTypeDismiss];
    transitioning.delegate = self;
    _transitioning = YES;
...
- (CLViewControllerTransitioningDelegate *)viewControllerTransitioningDelegate
{
    //是被present出来的
    if (self.presentingViewController) {
        ...
        if (delegate && [delegate isKindOfClass:[CLViewControllerTransitioningDelegate class]]) {
            return delegate;
        }else{
