...
- (id)initWithImageURL:(NSString *)URL
              delegate:(id<CLImageDownLoadDelegate>)delegate
        downLoadPolicy:(MyImageDownLoadPolicy)policy;


@property(nonatomic,copy,readonly) NSString *imageURL;
@property(nonatomic,strong,readonly) CLHTTPRequest * httpRequest;
@property(nonatomic,strong,readonly) NSMutableSet<CLWeakDelegate<id<CLImageDownLoadDelegate>> *> *delegateSet;

//缓存策略
@property(nonatomic,readonly) MyImageDownLoadPolicy policy;

//添加回调代理
- (void)addCallbackDelegate:(id<CLImageDownLoadDelegate>)delegate downLoadPolicy:(MyImageDownLoadPolicy)policy;
        ...
        downLoadPolicy:(MyImageDownLoadPolicy)policy
{
    if (self = [super init]) {
        ...
        _imageURL = [URL copy];
        _httpRequest = [[CLHTTPRequest alloc] initWithURL:URL];
        _policy = policy;

        //加入代理
        if (delegate != nil) {
            [self.delegateSet addObject:[[CLWeakDelegate alloc] initWithDelegate:delegate]];
        }
    }
...
- (NSMutableSet<CLWeakDelegate<id<CLImageDownLoadDelegate>> *> *)delegateSet
{
    if (!_delegateSet) {
...
- (void)unionTask:(_ImageDownLoadTask *)task
{
    assert([self.imageURL isEqualToString:task.imageURL]);

    //合并策略
    _policy |= task.policy;

    //加入代理
    for (CLWeakDelegate * delegate in task.delegateSet) {
...
- (void)removeInvaidDelegate
{
    ...
    for (CLWeakDelegate * delegate in self.delegateSet) {
        if (delegate.delegate == nil) {
...
- (id)initWithDelegate:(id<CLImageDownLoadDelegate>)delegate imageURL:(NSString *)imageURL;

@property(nonatomic,strong,readonly) CLWeakDelegate<id<CLImageDownLoadDelegate>> * delegate;
@property(nonatomic,copy,readonly) NSString * imageURL;
              ...
              forceCancle:(BOOL)forceCancle
{
    ...
    if (imageDownLoadTask) {
        ...
        if (delegate == nil && imageDownLoadTask.delegateSet.count == 0) { //不存在元素
            ...
            CLWeakDelegate * weakDelegate = [[CLWeakDelegate alloc] initWithDelegate:delegate];
...
@interface _CLImageDownLoadDelegateForBlock : NSObject <CLImageDownLoadDelegate>

+ (NSMutableSet *)delegateSet;
+ (instancetype)createDelegateWithSucceedBlock:(MyImageDownLoadSucceedBlock)succeedBlock
...
@implementation _CLImageDownLoadDelegateForBlock
...
+ (instancetype)createDelegateWithSucceedBlock:(MyImageDownLoadSucceedBlock)succeedBlock
                                   failedBlock:(MyImageDownLoadFailedBlock)failedBlock
{
    _CLImageDownLoadDelegateForBlock * delegate = [[self alloc] initWithSucceedBlock:succeedBlock failedBlock:failedBlock];
    [[self delegateSet] addObject:delegate];
...
- (void)imageDownLoadManager:(CLImageDownLoadManager *)manager
...
@interface CLImageDownLoadManager () <CLHTTPRequestDelegate,_ImageDownLoadWaitingPoolDelegate>
...
{
    static CLImageDownLoadManager * _shareImageDownLoadManager = nil;

    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _shareImageDownLoadManager = [[CLImageDownLoadManager alloc] init];
    });
...
- (id)initWithImageCachePool:(CLImageCachePool *)imageCachePool
...
- (CLImageCachePool *)imageCachePool
{
    if (!_imageCachePool) {
        static dispatch_once_t onceToken;
        dispatch_once(&onceToken, ^{
            _imageCachePool = [CLImageCachePool shareImageCachePool];
        });
    }
                                                     ...
                                                     error:(NSError **)pError
{
    ...
    if (UseLocalCache(policy)) { //从缓存获取
        resultImage = [self.imageCachePool imageWithKey:url policy:CLCacheImagePolicyDefault type:&imageCacheType];
    }
                ...
                if (resultImage == nil) {
                    ...
                    if (fileData && CacheImage(policy)) {
                        [self.imageCachePool cacheImage:resultImage
                                                    key:url
                                                 policy:CLCacheImagePolicyUseOuterCache
                                                  async:YES];
                    }
                }
...
- (void)startDownLoadImage:(id<CLImageDownLoadDelegate>)delegate
                       URL:(NSString *)url
            downLoadPolicy:(MyImageDownLoadPolicy)policy
{
    //核对图片URL
    ...
    if (AutoLoadCache(policy)) { //自动模式，如果无内存缓存则使用异步加载
        asyncLoad = ![self.imageCachePool hadCacheImageForKey:url policy:CLCacheImagePolicyUseOuterCache type:NULL];
...
- (id<CLImageDownLoadDelegate>)startDownLoadImageWithURL:(NSString *)url
                                          downLoadPolicy:(MyImageDownLoadPolicy)policy
                                            succeedBlock:(MyImageDownLoadSucceedBlock)succeedBlock
                                             failedBlock:(MyImageDownLoadFailedBlock)failedBlock
{
    //生成代理
    _CLImageDownLoadDelegateForBlock * delegate = [_CLImageDownLoadDelegateForBlock createDelegateWithSucceedBlock:succeedBlock failedBlock:failedBlock];
...
- (void)_startImageDownLoadTask:(_ImageDownLoadTask *)imageDownLoadTask
{
    ...
    //添加任务与代理的关联
    for (CLWeakDelegate * delegate in imageDownLoadTask.delegateSet) {
...
- (void)imageDownLoadWaitingPool:(_ImageDownLoadTaskWaitingPool *)waitingPool
                   didRemoveTask:(_ImageDownLoadTask *)imageDownLoadTask
{
    for (CLWeakDelegate * weakDelegate in imageDownLoadTask.delegateSet.allObjects) {
        id<CLImageDownLoadDelegate> delegate = weakDelegate.delegate;
...
- (void)cancleDownLoadImage:(id<CLImageDownLoadDelegate>)delegate forceToCancle:(BOOL)force {
    [self cancleDownLoadImage:delegate URL:nil forceToCancle:force];
}

- (void)cancleDownLoadImage:(id<CLImageDownLoadDelegate>)delegate
                        URL:(NSString *)url
              forceToCancle:(BOOL)force
{
    ...
    //移除代理
    if ([delegate isKindOfClass:[_CLImageDownLoadDelegateForBlock class]]) {
        [[_CLImageDownLoadDelegateForBlock delegateSet] removeObject:delegate];
    }
}
...
- (void)cancleAllDownLoadImage
{
    ...
    [_delegateToTasksMap removeAllObjects];
    [[_CLImageDownLoadDelegateForBlock delegateSet] removeAllObjects];
}
           ...
           receiveDataSpeed:(NSUInteger)speed
{
    ...
    if(downLoadTask){
        ...
        for (CLWeakDelegate * weakDelegate in downLoadTask.delegateSet.allObjects) {
...
- (void)_imageDownloadTask:(_ImageDownLoadTask *)task completedWithImage:(UIImage *)image error:(NSError *)error
{
    if (task) {
        ...
        if (image != nil && CacheImage(task.policy)) {
            [self.imageCachePool cacheImage:image key:task.imageURL policy:CLCacheImagePolicyDefault async:YES];
        }

        for (CLWeakDelegate * weakDelegate in task.delegateSet.allObjects) {
