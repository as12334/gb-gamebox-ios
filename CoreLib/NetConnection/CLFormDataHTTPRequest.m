...
@interface _CLUploadData : NSObject
...
@interface CLFormDataHTTPRequest ()
...
@implementation CLFormDataHTTPRequest
{
    //创建上传数据的标记
...
- (NSString *)boundary {
    return  _boundary ?: (_boundary = getUniqueID());
}
         ...
         forKey:(NSString *)key
{
    ...
    //添加
    _CLUploadData * uploadData = [[_CLUploadData alloc] initWithData:data
                                                                 key:key
                                                            fileName:fileName
                                                         contentType:contentType];
...
- (void)_addUploadData:(_CLUploadData *)uploadData
{
...
- (NSData *)_bodyDataWithUploadDatas:(NSArray *)uploadDatas
{
    ...
    for (_CLUploadData * uploadData in uploadDatas) {
...
@implementation CLFormDataHTTPRequest(image)
          ...
          forKey:(NSString *)key
{
    ...
    _CLUploadData * uploadData = [[_CLUploadData alloc] initWithImage:image
                                                                  key:key
                                                            imageName:imageName
                                                              quality:compressionQuality];
    [self _addUploadData:uploadData];
