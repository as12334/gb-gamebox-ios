...
#define xGap 5.0
#define wGap 2*xGap
@interface RH_ScrollSegmentView () {
...
@end
@implementation RH_ScrollSegmentView
...
- (void)titleLabelOnClick:(UITapGestureRecognizer *)tapGes {
    ...
    RH_CustomLabel *currentLabel = (RH_CustomLabel *)tapGes.view;
...
- (void)setupTitles {
    ...
    for (NSString *title in self.titles) {
        ...
        RH_CustomLabel *label = [[RH_CustomLabel alloc] initWithFrame:CGRectZero];
        label.tag = index;
        label.text = title;
...
- (void)setupUI {
    ...
    if (self.segmentStyle.isScrollTitle) { // 设置滚动区域
        RH_CustomLabel *lastLabel = (RH_CustomLabel *)self.titleLabels.lastObject;
...
- (void)setUpLabelsPosition {
    ...
    CGFloat titleY = 0.0;
    CGFloat titleW = 0.0;
    CGFloat titleH = self.rb_height - self.segmentStyle.scrollLineHeight;
    
    if (!self.segmentStyle.isScrollTitle) {// 标题不能滚动, 平分宽度
        titleW = _currentWidth / self.titles.count;
        
        NSInteger index = 0;
        for (RH_CustomLabel *label in self.titleLabels) {
        ...
        for (RH_CustomLabel *label in self.titleLabels) {
            ...
            if (index != 0) {
                RH_CustomLabel *lastLabel = (RH_CustomLabel *)self.titleLabels[index - 1];
    ...
    RH_CustomLabel *firstLabel = (RH_CustomLabel *)self.titleLabels[0];
...
- (void)setupScrollViewAndExtraBtn {
    ...
    self.scrollView.frame = CGRectMake(0.0, 0.0, scrollW, self.rb_height);
    if (self.extraBtn) {
        self.extraBtn.frame = CGRectMake(scrollW , extraBtnY, extraBtnW, self.rb_height - 2*extraBtnY);
    }
}
...
- (void)setupScrollLineAndCover {
    ...
    RH_CustomLabel *firstLabel = (RH_CustomLabel *)self.titleLabels[0];
    CGFloat coverX = firstLabel.rb_x;
    CGFloat coverW = firstLabel.rb_width;
    ...
    if (self.scrollLine) {
        ...
        if (self.segmentStyle.isScrollTitle) {
            self.scrollLine.frame = CGRectMake(coverX , self.rb_height - self.segmentStyle.scrollLineHeight, coverW , self.segmentStyle.scrollLineHeight);
            
        } else {
            if (self.segmentStyle.isAdjustCoverOrLineWidth) {
                coverW = [self.titleWidths[_currentIndex] floatValue] + wGap;
                coverX = (firstLabel.rb_width - coverW) * 0.5;
            }
...
//    [dic setValue:_titles[_currentIndex] forKey:@"title"];
//    [dic setValue:[NSNumber numberWithInteger:_currentIndex] forKey:@"index"];
//    [self postNotificationName:ScrollPageViewDidShowThePageNotification object:nil userInfo:@{@"currentIndex": dic}];
//
//}
...
- (void)adjustUIWhenBtnOnClickWithAnimate:(BOOL)animated {
    ...
    RH_CustomLabel *oldLabel = (RH_CustomLabel *)self.titleLabels[_oldIndex];
    RH_CustomLabel *currentLabel = (RH_CustomLabel *)self.titleLabels[_currentIndex];
    ...
    [UIView animateWithDuration:animatedTime animations:^{
        ...
        if (weakSelf.scrollLine) {
            if (weakSelf.segmentStyle.isScrollTitle) {
                weakSelf.scrollLine.rb_x = currentLabel.rb_x;
                weakSelf.scrollLine.rb_width = currentLabel.rb_width;
            } else {
                if (self.segmentStyle.isAdjustCoverOrLineWidth) {
                    CGFloat scrollLineW = [self.titleWidths[_currentIndex] floatValue] + wGap;
                    CGFloat scrollLineX = currentLabel.rb_x + (currentLabel.rb_width - scrollLineW) * 0.5;
                    weakSelf.scrollLine.rb_x = scrollLineX;
                    weakSelf.scrollLine.rb_width = scrollLineW;
                } else {
                    weakSelf.scrollLine.rb_x = currentLabel.rb_x;
                    weakSelf.scrollLine.rb_width = currentLabel.rb_width;
                }
        ...
        if (weakSelf.coverLayer) {
            if (weakSelf.segmentStyle.isScrollTitle) {
                ...
                weakSelf.coverLayer.rb_x = currentLabel.rb_x - xGap;
                weakSelf.coverLayer.rb_width = currentLabel.rb_width + wGap;
            } else {
                if (self.segmentStyle.isAdjustCoverOrLineWidth) {
                    CGFloat coverW = [self.titleWidths[_currentIndex] floatValue] + wGap;
                    CGFloat coverX = currentLabel.rb_x + (currentLabel.rb_width - coverW) * 0.5;
                    weakSelf.coverLayer.rb_x = coverX;
                    weakSelf.coverLayer.rb_width = coverW;
                } else {
                    weakSelf.coverLayer.rb_x = currentLabel.rb_x;
                    weakSelf.coverLayer.rb_width = currentLabel.rb_width;
                }
            }
...
- (void)adjustUIWithProgress:(CGFloat)progress oldIndex:(NSInteger)oldIndex currentIndex:(NSInteger)currentIndex {
    ...
    RH_CustomLabel *oldLabel = (RH_CustomLabel *)self.titleLabels[oldIndex];
    RH_CustomLabel *currentLabel = (RH_CustomLabel *)self.titleLabels[currentIndex];
    
    CGFloat xDistance = currentLabel.rb_x - oldLabel.rb_x;
    CGFloat wDistance = currentLabel.rb_width - oldLabel.rb_width;
    
    if (self.scrollLine) {
        
        if (self.segmentStyle.isScrollTitle) {
            self.scrollLine.rb_x = oldLabel.rb_x + xDistance * progress;
            self.scrollLine.rb_width = oldLabel.rb_width + wDistance * progress;
        } else {
            if (self.segmentStyle.isAdjustCoverOrLineWidth) {
                ...
                CGFloat oldScrollLineX = oldLabel.rb_x + (oldLabel.rb_width - oldScrollLineW) * 0.5;
                CGFloat currentScrollLineX = currentLabel.rb_x + (currentLabel.rb_width - currentScrollLineW) * 0.5;
                xDistance = currentScrollLineX - oldScrollLineX;
                self.scrollLine.rb_x = oldScrollLineX + xDistance * progress;
                self.scrollLine.rb_width = oldScrollLineW + wDistance * progress;
            } else {
                self.scrollLine.rb_x = oldLabel.rb_x + xDistance * progress;
                self.scrollLine.rb_width = oldLabel.rb_width + wDistance * progress;
            }
    ...
    if (self.coverLayer) {
        if (self.segmentStyle.isScrollTitle) {
            self.coverLayer.rb_x = oldLabel.rb_x + xDistance * progress - xGap;
            self.coverLayer.rb_width = oldLabel.rb_width + wDistance * progress + wGap;
        } else {
            if (self.segmentStyle.isAdjustCoverOrLineWidth) {
                ...
                CGFloat oldCoverX = oldLabel.rb_x + (oldLabel.rb_width - oldCoverW) * 0.5;
                CGFloat currentCoverX = currentLabel.rb_x + (currentLabel.rb_width - currentCoverW) * 0.5;
                xDistance = currentCoverX - oldCoverX;
                self.coverLayer.rb_x = oldCoverX + xDistance * progress;
                self.coverLayer.rb_width = oldCoverW + wDistance * progress;
            } else {
                self.coverLayer.rb_x = oldLabel.rb_x + xDistance * progress;
                self.coverLayer.rb_width = oldLabel.rb_width + wDistance * progress;
            }
        }
    }
...
- (void)adjustTitleOffSetToCurrentIndex:(NSInteger)currentIndex {
    ...
    CGFloat extraBtnW = self.extraBtn ? self.extraBtn.rb_width : 0.0;
