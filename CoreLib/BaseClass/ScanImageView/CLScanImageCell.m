...
@interface CLScanImageCell () < UIScrollViewDelegate,CLImageDownLoadDelegate >
...
@implementation CLScanImageCell
{
    //双击时的最小缩放比例
...
- (void)displayImage:(CLScanImageData *)scanImageData forAnimationCalculation:(BOOL)forAnimationCalculation
{
    ...
    if (sourceImage == nil && scanImageData.sourceImageURL.length) { //尝试从内存加载原图
        sourceImage = [[CLImageCachePool shareImageCachePool] imageWithKey:scanImageData.sourceImageURL
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        //更新显示状态
        if (sourceImage) {
            [self _setImageDisplayState:CLScanImageDisplayStateSource];
        }else {
            [self _setImageDisplayState:scanImageData.thumb ? CLScanImageDisplayStateThumb : CLScanImageDisplayStateNone];
...
{
    if (self.imageDisplayState == CLScanImageDisplayStateSource || self.scanImageData.sourceImageURL.length == 0) {
        return;
    }

    //设置状态
    [self _setImageDisplayState:CLScanImageDisplayStateLoading | self.imageDisplayState];
    ...
    CLActivityIndicatorView * activityIndicatorView = [[CLActivityIndicatorView alloc] init];
    ...
    //没有文件缓存则显示进度加载视图
    if(![[CLImageCachePool shareImageCachePool] hadCacheImageForKey:self.scanImageData.sourceImageURL]){
        activityIndicatorView.style = CLActivityIndicatorViewStyleDeterminate;
    }else {
    ...
    //开始下载图片
    [[CLImageDownLoadManager shareImageDownLoadManager] startDownLoadImage:self
...
- (void)_cancleLoadImage
{
    ...
    //取消下载
    [[CLImageDownLoadManager shareImageDownLoadManager] cancleDownLoadImage:self forceToCancle:NO];
            ...
            receiveDataSpeed:(NSUInteger)speed
{
    ...
    if (expectedDataLength != NSURLResponseUnknownLength) { //设置进度
        ...
        CLActivityIndicatorView * activityIndicatorView = (id)progressView.customView;
        activityIndicatorView.style = CLActivityIndicatorViewStyleIndeterminate;
        [activityIndicatorView startAnimating];
    }
}
                       ...
                       error:(NSError *)error
{
    ...
    [MBProgressHUD hideHUDForView:self animated:NO];
    [self _setImageDisplayState:self.imageDisplayState & ~CLScanImageDisplayStateLoading];
}

- (void)_netReachabilityChangedNotification:(NSNotification *)notification
{
    [[NSNotificationCenter defaultCenter] removeObserver:self name:NT_NetReachabilityChangedNotification object:nil];
    [self _startLoadSourceImage];
}
...
- (void)_setImageDisplayState:(CLScanImageDisplayState)imageDisplayState
{
    if (_imageDisplayState != imageDisplayState) {
        _imageDisplayState = imageDisplayState;

        if (imageDisplayState == CLScanImageDisplayStateNone){
            self.imageView.image = ImageWithName(@"assets_placeholder_picture") ;
        }

        id<CLScanImageCellDelegate> delegate = self.delegate;
...
- (void)_tapGestureRecognizerHandle:(UITapGestureRecognizer *)tapGestureRecognizer
{
    if (tapGestureRecognizer.numberOfTapsRequired == 1) {
        ...
        if ((self.imageDisplayState == CLScanImageDisplayStateThumb || self.imageDisplayState == CLScanImageDisplayStateSource)) {
