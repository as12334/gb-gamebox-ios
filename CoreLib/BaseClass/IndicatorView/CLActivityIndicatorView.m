...
@interface CLActivityIndicatorView ()<CAAnimationDelegate>
...
- (id)init {
    return [self initWithStyle:CLActivityIndicatorViewStyleIndeterminate];
}

- (id)initWithFrame:(CGRect)frame
{
    self = [self initWithStyle:CLActivityIndicatorViewStyleIndeterminate];
...
- (void)_init_MyActivityIndicatorView
{
    ...
    self.hidden = _style == CLActivityIndicatorViewStyleIndeterminate;
...
- (void)setStyle:(CLActivityIndicatorViewStyle)style
{
    if (_style != style) {
        _style = style;
        [self _reset];
...
- (void)_reset
{
    ...
    //设置隐藏与否
    self.hidden = _style == CLActivityIndicatorViewStyleIndeterminate && _hidesWhenStopped;

    //设置进度
    _progress = (_style == CLActivityIndicatorViewStyleIndeterminate && !_twoStepAnimation) ? _indeterminateProgress : 0.f;
    _shapeLayer.strokeEnd = _progress;
}
...
- (void)setIndeterminateProgress:(float)indeterminateProgress
{
    ...
    if (_indeterminateProgress != indeterminateProgress) {
        ...
        if (_style == CLActivityIndicatorViewStyleIndeterminate && (!_twoStepAnimation || _isAnimating)) {
...
{
    if (_style == CLActivityIndicatorViewStyleDeterminate) {
...
- (void)startAnimating
{
    if (_style == CLActivityIndicatorViewStyleIndeterminate) {
        [self stopAnimating];
