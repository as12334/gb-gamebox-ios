...
@interface _CLScanImageViewDataSource : NSObject<CLScanImageViewDataSource>

- (id)initWithImages:(NSArray<CLScanImageData *> *)imageDatas;
@property(nonatomic,strong,readonly) NSArray<CLScanImageData *> * imageDatas;

@end

//----------------------------------------------------------

@implementation _CLScanImageViewDataSource
...
@interface CLScanImageView () < CLPageViewDelegate,
CLPageViewDatasource,
CLScanImageCellDelegate,
...
//背景视图，用于显示背景色和毛玻璃效果
@property(nonatomic,strong) CLBlurredView * backgroundView;
...
@implementation CLScanImageView
...
{
    if (_overlayView) {
        _overlayView.frame = [_overlayView frameThatFitForScanImageOverlayView:self.bounds];
    }
}
    ...
    if (self.isScanning) {
        ...
        if (self.displayIndexIndicater && self.indexIndicaterMinimumDisplayImageCount <= self.numberOfImages) {
            if (!self.indexIndicaterLabel) {
                ...
                //计算大小
                CGSize indexIndicaterLabelSize = caculaterLabelTextDrawSize([NSString stringWithFormat:@"%i/%i",(int)self.numberOfImages,(int)self.numberOfImages], self.indexIndicaterLabel.font, MAXFLOAT);
...
{
    if (self.indexIndicaterLabel) {
        self.indexIndicaterLabel.center = CGPointMake(CGRectGetWidth(self.bounds) * 0.5f, 20.f + CGRectGetHeight(self.indexIndicaterLabel.bounds) * 0.5f);
    }
}
...
- (void)startScanImageAtIndex:(NSUInteger)index
               ...
               completedBlock:(void (^)())completedBlock
{
    _CLScanImageViewDataSource * dataSource = [[_CLScanImageViewDataSource alloc] initWithImages:images];
               ...
               completedBlock:(void(^)())completedBlock
{
    ...
    self.s_window.windowLevel = baseWindow.windowLevel;

    //毛玻璃视图
    self.backgroundView = [[CLBlurredView alloc] initWithFrame:self.s_window.bounds];
    ...
    //加入window
    self.frame = self.s_window.bounds;
    self.autoresizingMask = UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth;
    [self.s_window addSubview:self];
    ...
    //初始化page视图
    self.pageView = [[CLPageView alloc] initWithFrame:self.bounds];
    ...
    //注册复用
    [self.pageView registerCellForPage:[CLScanImageCell class] andReuseIdentifier:defaultReuseDef];
    ...
    if (animated) {
        ...
        CLScanImageData * imageData = [self _imageDataAtIndex:self.currentDispalyImageIndex];
        CLScanImageCell * cell = [[CLScanImageCell alloc] initWithFrame:self.bounds];
        ...
        if (CGRectEqualToRect(thumbShowFrame, CGRectZero)) { //无缩略图位置，显示简单动画
            ...
            //缩放后的缩略图大小
            CGSize scaledThumbSize = sizeZoomToTagetSize(thumbShowFrame.size, imageDisplayFrame.size, CLZoomModeAspectFit);
    ...
    //开始浏览
    id<CLScanImageViewDelegate> delegate = self.delegate;
...
- (NSUInteger)_numberOfImages
{
    ...
    id<CLScanImageViewDataSource> dataSource = self.dataSource;
...
- (CLScanImageData *)_imageDataAtIndex:(NSUInteger)index
{
    if (self.numberOfImages <= index) { //索引应该小于数据总数
        return nil;
    }

    CLScanImageData * image = [self.dataSource scanImageView:self imageForScanAtIndex:index withContext:self.scanContext];
    if (image == nil) {
        @throw [NSException exceptionWithName:NSInternalInconsistencyException
...
- (CLScanImageData *)imageAtIndex:(NSUInteger)index
{
    if (self.scanning) {
        ...
        CLScanImageCell * cell = [self.pageView cellForPageAtIndex:index];
        if (cell != nil) {
...
{
    CLScanImageCell * cell = [pageView dequeueReusableCellWithReuseIdentifier:defaultReuseDef forPageIndex:pageIndex];
    cell.delegate = self;
...
- (void)pageView:(CLPageView *)pageView didDisplayPageAtIndex:(NSUInteger)pageIndex
{
    ...
    ifRespondsSelector(delegate, @selector(scanImageView:didDisplayImageAtIndex:)) {
        [delegate scanImageView:self didDisplayImageAtIndex:pageIndex];
    }
...
- (void)endScanImageWithAnimated:(BOOL)animated completedBlock:(void (^)())completedBlock
{
    if (self.isScanning) {
        ...
        void (^_completedBlock)() = ^ {
            ...
            //初始化数据
            _dataSource = nil;
            _imageDisplayState = CLScanImageDisplayStateNone;
            _currentDispalyImageIndex = NSNotFound;
            _numberOfImages = 0;
        ...
        if (animated) {
            ...
            //获取当前显示的图像数据
            CLScanImageCell * cell = [self.pageView cellForPageAtIndex:self.currentDispalyImageIndex];
            CLScanImageData * imageData = cell.scanImageData;
...
- (BOOL)_isCurrentDispalyImageCell:(CLScanImageCell *)cell {
...
- (void)_changeImageDisplayState:(CLScanImageDisplayState)imageDisplayState
{
    if (_imageDisplayState != imageDisplayState) {

        CLScanImageDisplayState fromState = _imageDisplayState;
...
{
    return self.canLongPerssSaveImageToPhotosAlbum && self.isScanning && (self.imageDisplayState == CLScanImageDisplayStateThumb || self.imageDisplayState == CLScanImageDisplayStateSource);
}
...
@implementation UIView (CLScanImageOverlayView)

- (CLScanImageView *)scanImageView
{
    if ([self isKindOfClass:[CLScanImageView class]]) {
        return (id)self;
    }else {
